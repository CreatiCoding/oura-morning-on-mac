...
> [!TIP]
> **아래 3가지를 모두 충족**하면 깨웁니다 (`HEALTHY_MODE=1`):
> - 💤 **총 수면** ≥ `TARGET_SLEEP_HOURS` (기본 **8시간**)
> - 🧠 **REM 수면** ≥ `REM_MIN_MIN` (기본 **70분**)
> - 🛌 **깊은 수면** ≥ `DEEP_MIN_MIN` (기본 **55분**)
>
> 못 채우면 **안전 상한 시각**(`CAP_TIME`, 기본 09:00)에 **무조건** 깨웁니다 → 늦잠 걱정 없음.

> [!NOTE]
> - `HEALTHY_MODE=0`이면 **총 수면 시간만**으로 판정합니다.
> - 성인 건강 권장치(참고): REM 90–120분(20–25%), 깊은수면 70–120분(15–25%).
>   기본값은 achievable하게 낮춰뒀으니, 며칠 로그(`logs/wakeready.jsonl`)로 **본인 기준에 맞게 상향 보정**하세요.
> - REM/깊은수면은 원시신호(HR·HRV·모션) 기반 **추정치**입니다(공식 모델 없음).
...
- **Mac** — 침대에서 BLE가 닿는 거리(약 5m)에 두고 밤새 켜둘 것 (헤드리스 Mac mini도 OK)
