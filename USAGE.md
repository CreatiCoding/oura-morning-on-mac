...
- 헤드라인 총수면 / 목표 + 판정(😴/🙂/😐) + 취침→기상 (최신 밤이면 "라이브" 점, 5초마다 갱신)
- 5분 히프노그램(깸/REM/얕은/깊은, 정각 눈금, 터치·키보드 크로스헤어 툴팁) + 밑에 단계 합계 한 줄(REM·깊은·얕은·깸 분)
- "지금 동기화"·세션 상태는 최신 밤(라이브)에서만 보임
- **"자세히" 접기(기본 닫힘)**: 출처 배지·효율·모드, 같은 밤에 공식과 로컬이 둘 다 있으면 단계별 비교표,
  심박수 5분 라인(최저/최고/평균), 5분 표. 첫 화면은 숫자·판정·차트만 보이도록 나머지를 여기로 모았다(2026-09-09).
